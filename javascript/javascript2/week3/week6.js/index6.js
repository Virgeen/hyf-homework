...
let numbers = [1, 2, 3, 4];
 ...
 const myNumbers = numbers.map(numbers => {
    let newNumbers = [];
...
const moviesBigerThanSix = moview
.filter(rate =>  (rate.rating > 6))
.reduce(rate =>(rate.rating ) )

console.log(moviesBigerThanSix);


// map the movies array to only the rating of the movies
const justRateOfMovies = moviesBigerThanSix.map(rate => (rate.rating));

console.log(justRateOfMovies);


//4.Count he number of movies
const he = ["The", "dog", "who", "is", "not", "a", "man"];
// making sure  if the search is case insensitive? Hint: use indexOf
const moviesTitle = moview.map(movies => movies.title);

console.log(moviesTitle);
//combare tow arrays

/*
function diff(moviesTitle, he) {
    
  var compareNumberArray =[];  
  for (var i = 0; i < moviesTitle.length; i++) {
     if (he.indexOf(moviesTitle[i]) === -1){
     compareNumberArray.push(moviesTitle[i]);
     }
    }

  for(var j = 0; he.length; j++) {
      if (moviesTitle.indexOf(he[j]) ===-1) {
          compareNumberArray.push(he[j]);
      }
  }
...
let set = new Set([...prepareArray(he), ...prepareArray(moviesTitle)]);
let newArray = [...set];
